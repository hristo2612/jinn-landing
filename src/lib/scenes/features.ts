/**
 * Canonical features-page data - the single source of truth for every string
 * and semantic state on /features. Every string here comes verbatim from
 * STORYBOARD-FEATURES.md §4/§11 (the closed copy deck); the features surfaces
 * render from this data and never hardcode copy. Unit and e2e assertions
 * prove the shipped DOM and the scene reducer both equal these shapes.
 *
 * Fiction ledger (STORYBOARD-FEATURES §10.8): runs #147/#148, orders
 * #8841/#8867, times 10:56 → 14:05, the Support hire - no other numbers,
 * names, or times may appear on the page.
 */
import type {
  ChatMessage,
  ChatSceneState,
...
export const FEATURES_LINKS = {
  github: "https://github.com/hristo2612/jinn",
  npm: "https://www.npmjs.com/package/jinn-cli",
  landing: "/",
  features: "/features/",
} as const;
...
export const FEATURES_CTA = {
  ...
  command: "npm install -g jinn-cli",
  links: [
    { label: "GitHub", href: FEATURES_LINKS.github },
    { label: "npm", href: FEATURES_LINKS.npm },
  ],
} as const;

/* ── Section copy + fact rails (§4, verbatim) ──────────────────────────── */

export interface FactRailRow {
  label: string;
  text: string;
}
...
export const FEATURES_SECTIONS: Record<
  | "engines"
  | "employees"
  | "todos"
  | "workflows"
...
/** §08 static dashboard surface chip row. */
...
export const TREE_CARD = {
  ...
  rows: [
    ...
    { branch: "├──", name: "org/", comment: "# employees, one YAML each" },
    { branch: "├──", name: "skills/", comment: "# playbooks, one folder each" },
    {
      branch: "├──",
      name: "cron/jobs.json",
      comment: "# schedules, hot-reloaded",
    },
    { branch: "├──", name: "knowledge/", comment: "# what the company knows" },
    { branch: "└──", name: "sessions/", comment: "# history, SQLite" },
  ],
} as const;

/* ── §01 engine-switch - chat pane data ────────────────────────────────── */

export const ENGINE_PILL = {
  name: "Dev",
  role: "· Senior ·",
...
export const engineUserReview: ChatMessage = {
  kind: "message",
  id: "eng-user-1",
  target: "msg-user-1",
  role: "user",
  body: [{ text: "Review the payments webhook diff before we ship it." }],
};

export const engineDevFirst: ChatMessage = {
  kind: "message",
  id: "eng-dev-1",
  target: "msg-dev-1",
  role: "assistant",
...
export const engineUserRecheck: ChatMessage = {
  kind: "message",
  id: "eng-user-2",
  target: "msg-user-2",
  role: "user",
  body: [{ text: "Fresh eyes - re-check it." }],
};

export const engineTyping: TypingIndicator = {
  kind: "typing",
  target: "thread-typing",
  hidden: true,
};

export const engineDevSecond: ChatMessage = {
  kind: "message",
  id: "eng-dev-2",
  target: "msg-dev-2",
  role: "assistant",
...
export const engineSwitchInitial: ChatSceneState = {
  pane: "none",
  pill: {
    name: ENGINE_PILL.name,
    meta: `${ENGINE_PILL.role} ${ENGINE_PILL.initialEngine}`,
    presence: ENGINE_PILL.presence,
  },
  composerPlaceholder: "Message Dev…",
  thread: [engineUserReview, engineDevFirst, engineTyping],
};

export const engineSwitchResolved: ChatSceneState = {
  pane: "none",
  pill: {
    name: ENGINE_PILL.name,
    meta: `${ENGINE_PILL.role} ${ENGINE_PILL.engine}`,
    presence: ENGINE_PILL.presence,
  },
  composerPlaceholder: "Message Dev…",
  thread: [
...
/* ── §02 org-hire - org pane data ──────────────────────────────────────── */

/**
 * The features org opens at the landing's five-seat org and grows the
 * Support seat. No presence dots - the app's org map renders no
 * live-session indicator (Addendum B2-4; employee-node.tsx at the pin), so
 * the payoff is the seat itself. Employee identities are the landing's
 * canonical five plus Support - avatar from the app's own emojiForName hash
 * (Support → 🌊, same hash that yields the established five).
 */
export const ORG_HIRE_PANE = {
...
} satisfies OrgPaneData & { initialHeader: string };

/* ── §03 todo-approval - todos pane data ───────────────────────────────── */

export type FeatureTodoStatus = "in-review" | "executing" | "assigned";

export interface FeatureTodoCard {
  target: `card-${string}`;
  emoji: string;
  title: string;
  owner: string;
  status: FeatureTodoStatus;
  /** The app's own state label (packages/web/src/lib/todos.ts), serialized. */
  statusLabel: string;
}

export const TODO_APPROVAL_PANE = {
  header: "Todos",
  segments: [
    { target: "tab-active", label: "Active", count: "3" },
    { target: "tab-needs", label: "Needs you", count: "1" },
    { target: "tab-people", label: "People", count: undefined },
  ],
  cards: [
    {
      target: "card-refund",
  ...
  ] satisfies FeatureTodoCard[],
  badge: "Approval requested",
  request: "Card was double-charged - refund $49 to order #8841.",
  requester: { emoji: "🌊", name: "Support" },
  approveLabel: "Approve",
  sendBackLabel: "Send back",
  decidedStatus: "Approved by you · 11:42 - Todo done",
} as const;

/* ── §04 triage-run - canvas-mini data ─────────────────────────────────── */

export type CanvasNodeStatus =
  "pending" | "running" | "done" | "awaiting-approval";

export interface CanvasMiniNode {
  target: `node-${string}`;
  kind: "trigger" | "step" | "switch" | "gate";
  title: string;
  /** Small mono line under a glyph node, outside the box (trigger only). */
  sub?: string;
  /** Fired timestamp label revealed with the done state (trigger only). */
  fired?: string;
  status: CanvasNodeStatus;
  initialStatus: CanvasNodeStatus;
}

/** Plain-language node status vocabulary - packages/web status-line.ts, verbatim. */
export const CANVAS_STATUS_LINES: Record<CanvasNodeStatus, string> = {
  pending: "Up next",
  running: "Running",
  done: "Done",
...
export const TRIAGE_RUN_PANE = {
  title: "Support triage",
  lenses: ["Editor", "Executions"] as const,
  ...
  ] satisfies CanvasMiniNode[],
} as const;

/* ── §05 webhook-fire - triggers pane data ─────────────────────────────── */

export interface FeatureTriggerBinding {
  target: `binding-${string}`;
  title: string;
  detail: string;
  kind: "cron" | "todo-status" | "webhook" | "workflow-call";
  status: "idle" | "fired";
  initialStatus: "idle" | "fired";
  fired?: string;
  token?: string;
}

export const WEBHOOK_FIRE_PANE = {
  ...
  ] satisfies FeatureTriggerBinding[],
  run: "↳ run #148 started - Support triage",
} as const;

/* ── §06 mcp-hands - feed card data ────────────────────────────────────── */
...
export const MCP_HANDS_CARD = {
  ...
  ] satisfies McpCallRow[],
} as const;

/* ── §07 slack-approve - connector-card data ───────────────────────────── */

export const SLACK_APPROVE_CARD = {
...
/* ── Scene captions (a11y transcripts, §4 verbatim) ────────────────────── */

export const FEATURES_CAPTIONS = {
  "engine-switch":
